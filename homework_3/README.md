...
Порт Gi0/1 на S2 отображается в качестве альтернативного и в настоящее время заблокирован. Протокол spanning-tree выбрал этот порт в качестве невыделенного (заблокированного) порта, потому что коммутатор S2 имеет наибольший Bridge ID, поэтому блокировка порта будет происходить на нем. Блокируется порт Gi0/1, т.к. он назначенный. 
